<script setup>
definePageMeta({
  title: '１月のアクセス',
  layout: 'jimmy',
  team: 'topics',
});

// レイアウトから提供された 'pageTheme' を受け取る
const themeRaw = inject('jimmyColor', null);

// 2. computed を使って安全に中身を取り出す
// .value が存在しない場合（default）のフォールバックを用意する
const themeConfig = computed(() => {
  return themeRaw?.value || {
    bgimg: 'bg-[url(/images/jimmy/review/sonota.gif)]',
    bgcolor: 'bg-slate-200',
    bar: 'bg-slate-400',
    map: 'bg-slate-500',
    barborder: 'border-slate-500',
    texttitle: 'text-slate-600',
    textlink: 'text-slate-500',
  };
});
</script>

<template>
  <div>
    <div class="flex justify-center pl-4 lg:pl-8 pr-4 lg:pr-8 pt-20">
      <div class="w-48 flex-none pb-16 hidden lg:block">
        <JimmyMenu />
      </div>
      <div class="flex-1 pl-2 md:pl-8 xl:pl-16 pr-2 md:pr-8 xl:pr-16 pb-16">
        <div class="pt-8">
          <h2
            class="text-2xl sm:text-3xl md:text-4xl klee-one font-[400] mb-8"
            :class="themeConfig.texttitle"
          >
            １月のアクセス
          </h2>
          <div class="text-gray-800 text-base md:text-lg leading-8 [&>p]:mb-4">
            <p
              class="text-right"
              :class="themeConfig.texttitle"
            >
              (1999/02/06 up)
            </p>
            <p>プロバイダーからの報告書より、今月も皆様に少しだけお知らせしますね。()内が１月のアクセス数です。</p>
            <p>●トップページ(5468)</p>
            <p>～沢山のご来場、本当にありがとうございます！！</p>
            <p>●トピックス/最新(1517)</p>
            <p>～真剣なものから、小ネタ、バカネタまで（^_^;）、時事に沿って思いのままに書いておりますので、是非最近お越しになった方も、最初から読んで頂きたいです。</p>
            <p>●まずは公演(1232)</p>
            <p>1.黒い瞳/東京公演(646)2.夜明けの序曲(323)3.聖夜物語(192)</p>
            <p>～やはり代役報告が効いたのか（^_^;）、黒い瞳/東京公演がダントツ人気ですね。</p>
            <p>●気になるこの人をチェック(747)</p>
            <p>組1.宙組(179)2.花組(175)3.月組(143)</p>
            <p>生徒1.姿月あさと(142)2.汐美真帆(130)3.檀れい(123)</p>
            <p>～相変わらず、宙組とずんちゃん(姿月)が人気ですね～（^_^;）。ありがとうございます（^_^;）。ケロちゃん(汐美)もようやくランクインしました。１月は大騒ぎだったものね（^_^;）。檀ちゃんもここの所ずーっと多くて、注目度の高さが伺えます。</p>
            <p>●ＪＩＭＭＹの投稿文大公開(353)</p>
            <p>1.組替え/檀・汐美など(137)2.組替え/真琴・姿月など(61)3.エクスカリバー(60)</p>
            <p>～組替えネタが人気ですね（^_^;）。</p>
          </div>
        </div>
      </div>
      <div class="w-80 flex-none pt-8 pb-16 hidden lg:block">
        <JimmyTopicsList />
      </div>
    </div>
    <div class="w-full flex-none pt-8 pb-16 lg:hidden pl-8 pr-8">
      <JimmyTopicsList />
    </div>
  </div>
</template>

<style scoped></style>
