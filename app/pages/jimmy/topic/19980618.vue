<script setup>
definePageMeta({
  title: '「宙組特集本」読みました',
  layout: 'jimmy',
  team: 'topics',
});

// レイアウトから提供された 'pageTheme' を受け取る
const themeRaw = inject('jimmyColor', null);

// 2. computed を使って安全に中身を取り出す
// .value が存在しない場合（default）のフォールバックを用意する
const themeConfig = computed(() => {
  return themeRaw?.value || {
    bgimg: 'bg-[url(/images/jimmy/review/sonota.gif)]',
    bgcolor: 'bg-slate-200',
    bar: 'bg-slate-400',
    map: 'bg-slate-500',
    barborder: 'border-slate-500',
    texttitle: 'text-slate-600',
    textlink: 'text-slate-500',
  };
});
</script>

<template>
  <div>
    <div class="flex justify-center pl-4 lg:pl-8 pr-4 lg:pr-8 pt-20">
      <div class="w-48 flex-none pb-16 hidden lg:block">
        <JimmyMenu />
      </div>
      <div class="flex-1 pl-2 md:pl-8 xl:pl-16 pr-2 md:pr-8 xl:pr-16 pb-16">
        <div class="pt-8">
          <h2
            class="text-2xl sm:text-3xl md:text-4xl klee-one font-[400] mb-8"
            :class="themeConfig.texttitle"
          >
            「宙組特集本」読みました
          </h2>
          <div class="text-gray-800 text-base md:text-lg leading-8 [&>p]:mb-4">
            <p
              class="text-right"
              :class="themeConfig.texttitle"
            >
              (1998/06/18 up)
            </p>
            <p>結局<strong>各組５冊共</strong>買ってしまいました(^_^;)。</p>
            <p>お気に入りは<strong>「間違い探し」</strong>のコーナー。<strong>ｔａｐに混ざる組長</strong>とか、<strong>入れ替わる寿＆達姉妹</strong>とか、ツボを押さえてますね～(^_^;)。あと、<strong>ずんちゃん(姿月あさと)</strong>の<strong>赤ちゃんの頃の写真</strong>も、めちゃくちゃ可愛い(^_^;)。姿月あさとのキーワードのコーナーも、<strong>「心の旅路」「マチウ」「夜霧のモンマルトル」</strong>等、懐かしいもの満載で、嬉しかったです(^_^;)。</p>
            <p>あと、前、同じような事を宝塚ＨＰに書いて怒られた事があるのに、<strong>又しょーこりもなく書いてしまいますが</strong>(^_^;)、<strong>夢輝のあ</strong>が主なメンバーに入っていて、ケロちゃん(汐美真帆)ファンとしてはやっぱり<strong>「くやし～」</strong>(私が悔しがっても仕方がないのですが(^_^;))と叫んでしまいました。<strong>好きなんですけどね。ねったん(夢輝のあ)も。</strong></p>
            <p>それから、話題の新人演出家<strong>荻田浩一先生</strong>がチラッと写っていたのですが、<strong>想像と全然ちがう容姿</strong>で、びっくりしましたー(^_^;)。</p>
          </div>
        </div>
      </div>
      <div class="w-80 flex-none pt-8 pb-16 hidden lg:block">
        <JimmyTopicsList />
      </div>
    </div>
    <div class="w-full flex-none pt-8 pb-16 lg:hidden pl-8 pr-8">
      <JimmyTopicsList />
    </div>
  </div>
</template>

<style scoped></style>
